...
def draw_labels(fig, ax, out_value, features, feature_type, offset_text, total_effect=0, min_perc=0.05, text_rotation=0):
    ...
    # Draw initial line
    if feature_type == 'positive':
        x, y = np.array([[pre_val, pre_val], [0, -0.18+new_offset]])
    ...
    for feature in features:
        ...
        text_out_val = plt.text(start_text - sign * offset_text,
                                -0.15+new_offset-0.03, text,
                                fontsize=13, color=colors[0], fontweight="bold",
        ...
        if (sign * box_end_) > (sign * val):
            x, y = np.array([[val, val], [0, -0.18+new_offset]])
            line = lines.Line2D(x, y, lw=1., alpha=0.5, color=colors[0])
            line.set_clip_on(False)
            ax.add_line(line)
            start_text = val
    ...
    path = [[out_value, 0], [pre_val, 0], [box_end, -0.08],
            [box_end, -0.2+new_offset], [out_value, -0.2+new_offset],
            [out_value, 0]]
    ...
    # Create shading
    if feature_type == 'positive':
        colors = np.array([(255, 13, 87), (55, 71, 79)]) / 255.
    else:
        colors = np.array([(30, 136, 229), (55, 71, 79)]) / 255.
    
    cm = matplotlib.colors.LinearSegmentedColormap.from_list('cm', colors)
    
    Z, Z2 = np.meshgrid(np.linspace(0, 2), np.linspace(-10, 2))
...
def draw_output_element(out_name, out_value, ax):
    ...
    # Add output value
    x, y = np.array([[out_value, out_value], [0, rel-0.01]])
    ...
    text_out_val = plt.text(out_value, rel-0.14, '{0:.2f}'.format(out_value),
                            #fontproperties=font, alpha=0.8,
                            fontsize=font_size,
                            horizontalalignment='center')
    text_out_val.set_bbox(dict(facecolor='white', edgecolor='white'))
    
    text_out_val = plt.text(out_value, rel, out_name,
...
def draw_base_element(base_value, ax):
    ...
    x, y = np.array([[base_value, base_value], [0.11, rel-0.01]])
    line = lines.Line2D(x, y, lw=2., color='#F2F2F2')
    line.set_clip_on(False)
    ax.add_line(line)
    
    text_out_val = plt.text(base_value, rel, 'base value',
                            fontsize=font_size,# fontweight="bold", #alpha=0.5,
                            horizontalalignment='center')
    text_out_val.set_bbox(dict(facecolor='white', edgecolor='white'))
...
def draw_higher_lower_element(out_value, offset_text):
    font_size = 15
    rel = 0.60
    plt.text(out_value - offset_text, rel+0.005, 'higher',
             fontsize=font_size, color='#FF0D57',
             horizontalalignment='right', fontweight="bold")

    plt.text(out_value + offset_text, rel+0.005, 'lower',
             fontsize=font_size, color='#1E88E5',
             horizontalalignment='left', fontweight="bold")
    
    plt.text(out_value, rel, r'$\leftarrow$',
             fontsize=font_size, color='#1E88E5',
             horizontalalignment='center')
    
    plt.text(out_value, rel+0.025, r'$\rightarrow$',
             fontsize=font_size, color='#FF0D57',
             horizontalalignment='center')
...
def update_axis_limits(ax, total_pos, pos_features, total_neg,
    ...
    ax.spines["top"].set_position(["axes", 0.48])
